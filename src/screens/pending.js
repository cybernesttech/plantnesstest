...
function _wireGroupActions(staffNames, groups) {
  ...
  // Individual approve — sync editable fields first
  document.querySelectorAll('[data-approve]').forEach(btn =>
    btn.addEventListener('click', async () => {
      const sid = btn.dataset.approve;
      _syncEditableFields(sid);
...
// Reads the editable inputs in the pending card and updates the
// in-memory _submissions array before approval.
function _syncEditableFields(sid) {
  const submission = _submissions.find(s => s.id === sid);
  if (!submission) return;

  submission.items = (submission.items || []).map((item, idx) => {
    ...
    return {
      ...
      variant_size: variant  ?? item.variant_size,
