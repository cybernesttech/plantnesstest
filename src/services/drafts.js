  ...
  try {
    ...
    const draftData = {
      ...
      items:       items.map(i => ({
        ...
        variant_size: i.variantSize || i.variant_size || '',
...
export async function approvePending(businessId, pendingId, { reviewedBy = '' } = {}) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      // ── IN items (default) ───────────────────────────────────────────
      // If variant_id is missing, try to find or create the product/variant
      let variantId = item.variant_id;
      let productId = item.product_id;

      if (!variantId) {
        // New product/variant submitted by staff — find or create it
        try {
          ...
          let prod  = prods.find(p => (p.name||'').toLowerCase() === name.toLowerCase());

          if (!prod) {
            const r = await dbAdd(p.products, {
              name, category: item.category || 'Plants',
              created_at: now, updated_at: now,
            });
            productId = r.id;
            await dbUpdate(p.products, productId, { productId });
            await loadCache(businessId);
            prods = searchProducts('');
            prod  = prods.find(p => (p.name||'').toLowerCase() === name.toLowerCase());
          } else {
            productId = prod.productId || prod.id;
          }

          // Find or create variant
          const vari = prod?.variants.find(v => (v.size||'').toLowerCase() === size.toLowerCase());
          if (!vari) {
            const vr = await dbAdd(p.variants, {
              product_id: productId, size, price: Number(item.price || 0),
              low_stock_threshold: 5, lastPhysicalCountAt: null,
              transactionCountSinceCount: 0, created_at: now, updated_at: now,
            });
            variantId = vr.id;
            await dbSet(p.inventoryState, variantId, {
          ...
          console.error('[drafts.approvePending] product/variant creation failed', e);
          skipped++; continue;
        }
      }
